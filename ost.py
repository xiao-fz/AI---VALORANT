...
class DDMouseController:
    ...
    def press_o(self):
        self.dd.DD_key(O_KEY_CODE, 1)
        time.sleep(O_HOLD_S)
        self.dd.DD_key(O_KEY_CODE, 2)
...
def grab_yolo_input(cam, cache, dtype, device, imgsz):
    ...
    if frame is None:
        if cache.get("valid") and cache.get("tensor") is not None:
            return cache["tensor"], cache.get("coord_scale", 1.0)
        return None, None
...
def run_detection_and_move(cam, model, backend, dtype, device, model_imgsz, cache,
    ...
    infer_input, coord_scale = grab_yolo_input(cam, cache, dtype, device, model_imgsz)
...
def main():
    ...
    try:
        cam, cap_info = auto_create_capture(region, timeout_ms=CAPTURE_TIMEOUT_MS, verbose=False)
    except Exception as e:
        print(f"截图初始化失败: {e}")
        return
