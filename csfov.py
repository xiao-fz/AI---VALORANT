...
# ==================== 配置区（只改这里） ====================
# 按键
VK_XBUTTON1 = 0x05          # 鼠标侧键后退，触发瞄准
VK_XBUTTON2 = 0x06          # 鼠标侧键前进，触发瞄准
POLL_INTERVAL_S = 0.001     # 侧键轮询间隔（秒），越小按键越跟手、越吃 CPU
...
MULTIPLIER_SCALE = 0.7      # 瞄准倍率微调，越大动得越猛
O_KEY_CODE = 309            # DD 驱动的 O 键码
O_HOLD_S = 0.02             # O 键按下保持时间（秒）

# 截图 / 模型
CAPTURE_SIZE = 640          # 屏幕中心截图边长（像素）
...
GRAB_WAIT_MS = 0            # 空轮询没帧时再等的毫秒，0=不等
# ==================== 配置区结束 ====================
...
class DDMouseController:
    ...
    def move_to_target(self, target_x, target_y):
        ...
        n = self._dd_movr(dx_dd, dy_dd)
        return True, n, dx_dd, dy_dd

    def press_o(self):
        down = self.dd.DD_key(O_KEY_CODE, 1)
        time.sleep(O_HOLD_S)
        up = self.dd.DD_key(O_KEY_CODE, 2)
        return down, up
...
def init_dd(dll_path):
    ...
    try:
        ...
        time.sleep(1)
        return dd_dll if dd_dll.DD_btn(0) == 1 else None
    except Exception:
        return None
...
def closest_screen_target(pred, x_start, y_start, screen_center_x, screen_center_y,
    ...
    tx, ty, conf = torch.stack((px[i], py[i], boxes[i, 4])).tolist()
    return int(tx), int(ty), float(conf), ""
...
def grab_yolo_input(cam, cache, dtype, device, imgsz):
    t0 = time.perf_counter()
    frame = _grab_once(cam)
    t_grab = (time.perf_counter() - t0) * 1000
    if frame is None:
        if cache.get("valid") and cache.get("tensor") is not None:
            return cache["tensor"], cache.get("coord_scale", 1.0), "reuse", t_grab, 0.0
        return None, None, None, t_grab, 0.0
    t1 = time.perf_counter()
    tensor, coord_scale = bgra_to_yolo_tensor(getattr(frame, "array", frame), dtype, device, imgsz=imgsz)
    if hasattr(frame, "release"):
        frame.release()
    t_prep = (time.perf_counter() - t1) * 1000
    cache["tensor"] = tensor
    cache["coord_scale"] = coord_scale
    cache["valid"] = True
    return tensor, coord_scale, "new", t_grab, t_prep
...
def run_detection_and_move(cam, model, backend, dtype, device, model_imgsz, cache,
                            mouse_controller, x_start, y_start, counter=0):
    counter += 1
    t_total_start = time.perf_counter()
    use_cuda = device.type == "cuda"

    infer_input, coord_scale, how, t_grab, t_prep = grab_yolo_input(cam, cache, dtype, device, model_imgsz)
    if use_cuda:
        t_sync = time.perf_counter()
        torch.cuda.synchronize()
        t_prep += (time.perf_counter() - t_sync) * 1000
    t_get = t_grab + t_prep
    if infer_input is None:
        t_total = (time.perf_counter() - t_total_start) * 1000
        print(f"[{counter}|SIDE] 截图失败 wait={t_grab:.1f}ms | 总耗时: {t_total:.1f}ms | 未瞄准")
        return counter

    t0 = time.perf_counter()
    pred = run_engine(backend, infer_input)
    tx, ty, conf, method = closest_screen_target(
    ...
    o_info = ""
    if tx is not None:
        _, steps, dx_dd, dy_dd = mouse_controller.move_to_target(tx, ty)
        cache["valid"] = False
        down, up = mouse_controller.press_o()
        o_info = f" | 按O down={down} up={up}"
    t_act = (time.perf_counter() - t0) * 1000

    t_total = (time.perf_counter() - t_total_start) * 1000
    if tx is not None:
        print(f"[{counter}|SIDE] 截图: {t_get:.1f}ms(wait {t_grab:.1f}+prep {t_prep:.1f},{how}) | "
              f"推理: {t_infer:.1f}ms | "
              f"动作: {t_act:.1f}ms dx={dx_dd} dy={dy_dd} 步={steps} | "
              f"总耗时: {t_total:.1f}ms | 置信度: {conf:.2f}{o_info}")
    else:
        print(f"[{counter}|SIDE] 截图: {t_get:.1f}ms(wait {t_grab:.1f}+prep {t_prep:.1f},{how}) | "
              f"推理: {t_infer:.1f}ms | "
              f"总耗时: {t_total:.1f}ms | 无目标 | 未按O")
    return counter
...
def main():
    ...
    try:
        ...
        print(
            f"YOLO模型加载成功，RapidShot GPU: 捕获 {CAPTURE_SIZE} → 推理 {MODEL_IMGSZ}，"
            f"推理设备 {device}，engine={precision}，"
            f"截图 Device {cap_info['device_idx']}/{cap_info['desc']} "
    ...
    last_side_state = False
    side_counter = 0
    print("脚本已启动！按键当场截图，不是视频流")
    print("  [鼠标侧键按下]  → 截图一次 + 移动 + 按 O")
    print("按 Ctrl+C 停止")

    try:
        while True:
            side_down = is_key_pressed(VK_XBUTTON1) or is_key_pressed(VK_XBUTTON2)
            if side_down and not last_side_state:
                side_counter = run_detection_and_move(
                    cam, model, backend, dtype, device, MODEL_IMGSZ, shot_cache,
                    mouse_controller, x_start, y_start,
                    counter=side_counter,
                )
            last_side_state = side_down
